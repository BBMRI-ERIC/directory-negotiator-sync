...
from eu.bbmri.directorysync.negotiator_client import create_resource_add_DTO, create_network_add_DTO, NegotiatorAPIClient
...
def sync_all(negotiator_client: NegotiatorAPIClient):
    ...
    sync_all_organizations(negotiator_client, directory_organizations, negotiator_organizations)
    sync_all_resources(negotiator_client, directory_resources, negotiator_resources)
    ...
    for directory_organization in directory_organziations:
        ...
        if negotiation_organization:
            if negotiation_organization.name != directory_organization.name:
                ...
                negotiator_client.update_organization_name(negotiation_organization.id, directory_organization.name, external_id)
        else:
            LOG.info(
...
def sync_all_resources(negotiator_client: NegotiatorAPIClient, directory_resources: list[ResourceDirectoryDTO],
...
def sync_all_networks(negotiator_client: NegotiatorAPIClient,directory_networks: list[NetworkDirectoryDTO], negotiator_networks: list[NegotiatorNetworkDTO]):
    ...
    for directory_network in directory_networks:
        ...
        if network:
            if network.name != directory_network.name or network.uri != directory_network.url or network.contactEmail != directory_network.contact.email:
                ...
                negotiator_client: NegotiatorAPIClient.update_network_info(network.id,directory_network.name, directory_network.url, directory_network.contact.email, external_id)
        else:
